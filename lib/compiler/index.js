...
class Compiler {
    ...
    compile_inner(scope) {
        let result = new statements_1.StatementList();
        this.nextToken();
        while (this.token != tokens_1.ENDOFFILE && this.token != null) {
            if (this.token.value == constants_1.KEYWORD_RETURN) {
                if (scope.isRoot()) {
                    throw new errors_1.SyntaxError('return outside of function');
                }
                this.nextToken();
                result.push(new statements_1.ReturnStatement(this.expresionSequence(scope)));
                ...
                if (this.rules.find(it => it.getName() === 'DisableIf')) {
                    throw new errors_1.CompilationError('If statement is disabled');
                }
                ...
                if (this.rules.find(it => it.getName() === 'DisableFor')) {
                    throw new errors_1.CompilationError('For statement is disabled');
                }
                this.nextToken();
                let name = this.token.value;
                this.nextToken();
                if (!scope.variableInCurrent(name)) {
                    scope.push(new context_1.Reference(name, 'variable'));
                }
                this.check(constants_1.OPERATOR_IN);
                this.check(constants_1.KEYWORD_RANGE);
                this.check(constants_1.PARENTHESES_LEFT);
                let start, step;
                ...
                this.check(constants_1.PARENTHESES_RIGHT);
                this.check(constants_1.SYMBOL_END_OF_DEFINITION, false);
                this.checkNewline({});
                this.checkIndent();
                if (start) {
                    result.push(new statements_1.ForLoopStatement(name, this.compile_inner(scope), start, stop, step));
                }
                else {
                    result.push(new statements_1.ForLoopStatement(name, this.compile_inner(scope), stop, start, step));
                }
                this.checkDedent();
                this.nextToken();
            }
                ...
                if (this.rules.find(it => it.getName() === 'DisableWhile')) {
                    throw new errors_1.CompilationError('While statement is disabled');
                }
                this.nextToken();
                let condition = new statements_1.ExpressionStatement(this.booleanOrOperation(scope));
                this.check(constants_1.SYMBOL_END_OF_DEFINITION, false);
                this.checkNewline({});
                this.checkIndent();
                result.push(new statements_1.WhileLoopStatement(condition, this.compile_inner(scope)));
                this.checkDedent();
                this.nextToken();
            }
            else if (this.token.value == constants_1.KEYWORD_DEF) {
                if (this.rules.find(it => it.getName() === 'DisableFunctionDef')) {
                    throw new errors_1.CompilationError('Function definition is disabled');
                }
                ...
                if (this.rules.find(it => it.getName() === 'DisablePrint')) {
                    throw new errors_1.CompilationError('Print is disabled');
                }
                this.checkLeftParenth();
                this.nextToken();
                let body = new statements_1.ExpressionStatement(this.expresionSequence(scope));
                if (this.token.value != constants_1.PARENTHESES_RIGHT)
                    throw new errors_1.SyntaxError('invalid syntax', this.token.value);
                this.nextToken();
                result.push(new statements_1.PrintStatement(body));
                if (this.checkNewline({ nextToken: false, checkEOF: true }))
                    return result;
                this.nextToken();
            }
                ...
                if (this.lexer.peekNextToken().value === constants_1.OPERATOR_EQUALS) {
                    this.nextToken();
                    this.nextToken();
                    const value = new statements_1.ExpressionStatement(this.booleanOrOperation(scope));
                    result.push(new statements_1.AssignmentStatement(name, value, scope.variableInParentTree(name)));
                    if (!scope.variableInCurrent(name)) {
                        scope.push(new context_1.Reference(name, 'variable'));
                    }
                    if (this.checkNewline({ nextToken: false, checkEOF: true }))
                    ...
                    result.push(new statements_1.ExpressionStatement(this.booleanOrOperation(scope)));
                    if (this.checkNewline({ nextToken: false, checkEOF: true }))
                        return result;
                    this.nextToken();
                }
    ...
    functionDefinitionOperation(scope, result) {
        this.nextToken();
        let name = this.token.value;
        this.nextToken();
        ...
        this.nextToken();
        let args = [];
        scope.push(new context_1.Reference(name, 'function'));
        ...
        while (this.token.value != constants_1.PARENTHESES_RIGHT) {
            ...
            localScope.push(new context_1.Reference(this.token.value, 'variable', false));
            this.nextToken();
            if (this.token.value == constants_1.SYMBOL_COMMA_DELIMIER) {
                this.nextToken();
            }
        ...
        result.push(new statements_1.FunctionDefinitionStatement(name, args, body, localScope));
        this.checkDedent();
        this.nextToken();
    }
    checkEquals() {
    ...
    ifOperation(scope) {
        let clauses = [];
        if (this.token.value == constants_1.KEYWORD_IF) {
            this.nextToken();
            let clauseExpression = new statements_1.ExpressionStatement(this.booleanOrOperation(scope));
            if (this.token.value != constants_1.SYMBOL_END_OF_DEFINITION)
                throw new errors_1.SyntaxError(`invalid syntax`);
            this.checkNewline({});
            this.checkIndent();
            let clauseBody = this.compile_inner(scope);
            clauses.push(new statements_1.Clause(clauseExpression, clauseBody));
            this.checkDedent();
            this.nextToken();
        }
        if (this.token.value == constants_1.KEYWORD_ELIF) {
            this.nextToken();
            let clauseExpression2 = new statements_1.ExpressionStatement(this.booleanOrOperation(scope));
            if (this.token.value != constants_1.SYMBOL_END_OF_DEFINITION)
                throw new errors_1.SyntaxError(`invalid syntax`);
            this.checkNewline({});
            this.checkIndent();
            clauses.push(new statements_1.Clause(clauseExpression2, this.compile_inner(scope)));
            this.checkDedent();
            this.nextToken();
        }
        if (this.token.value == constants_1.KEYWORD_ELSE) {
            ...
            this.checkNewline({});
            this.checkIndent();
            clauses.push(new statements_1.Clause(null, this.compile_inner(scope)));
            this.checkDedent();
            this.nextToken();
        }
        return new statements_1.IfStatement(clauses);
    }
    number(scope) {
        ...
        if (this.token instanceof tokens_1.Literal) {
            let result = new index_1.Value(index_1.ValueType[this.token.name], this.token);
            this.nextToken();
            return result;
        }
        else if (this.token instanceof index_1.Enclosure) {
            let result = new index_1.Atom(this.token);
            this.nextToken();
            return result;
        }
            ...
            if (this.token.value == constants_1.KEYWORD_TRUE || this.token.value == constants_1.KEYWORD_FALSE) {
                let result = new index_1.Value(index_1.ValueType.Boolean, this.token);
                this.nextToken();
                return result;
            }
            else if (this.token.value == constants_1.KEYWORD_NONE) {
                let result = new index_1.Value(index_1.ValueType.Null, this.token, 'null');
                this.nextToken();
                return result;
            }
            else {
            ...
            if (this.token.value === constants_1.PARENTHESES_LEFT) {
                ...
                if (this.token.value === constants_1.PARENTHESES_RIGHT) {
                    this.nextToken();
                    return new index_1.FunctionCallExpression(nameToken.value);
                }
                else {
                    ...
                    this.nextToken();
                    return new index_1.FunctionCallExpression(nameToken.value, sequenceExpression);
                }
            }
            return new index_1.AccessExpression(nameToken.value, nameToken);
        }
        else {
    ...
    bracket(scope) {
        if (this.token.value != constants_1.PARENTHESES_LEFT) {
            ...
            this.nextToken();
            let result = new index_1.Enclosure(this.booleanOrOperation(scope));
            if (this.token.value == constants_1.PARENTHESES_RIGHT) {
                this.nextToken();
            }
            else {
    ...
    negateOperation(scope) {
        if (this.token.value != constants_1.OPERATOR_SUBRTACT) {
            ...
            return new index_1.Negate(this.bracket(scope));
        }
    }
    powerOperation(scope) {
        ...
        while (true) {
            if (this.token.value == constants_1.OPERATOR_POWER) {
                this.nextToken();
                result = new index_1.Power(result, this.negateOperation(scope));
            }
            else {
                return result;
            }
    ...
    mulDivModOperation(scope) {
        ...
        while (true) {
            if (this.token.value == constants_1.OPERATOR_MULTIPLY) {
                this.nextToken();
                result = new index_1.Multiplication(result, this.powerOperation(scope));
            }
            else if (this.token.value == constants_1.OPERATOR_DIVISION) {
                this.nextToken();
                result = new index_1.Division(result, this.powerOperation(scope));
            }
            else if (this.token.value == constants_1.OPERATOR_MODULO) {
                this.nextToken();
                result = new index_1.Modulo(result, this.powerOperation(scope));
            }
            else {
                return result;
            }
    ...
    addSubOperation(scope) {
        ...
        while (true) {
            if (this.token.value == constants_1.OPERATOR_ADD) {
                this.nextToken();
                result = new index_1.Addition(result, this.mulDivModOperation(scope));
            }
            else if (this.token.value == constants_1.OPERATOR_SUBRTACT) {
                this.nextToken();
                result = new index_1.Subtraction(result, this.mulDivModOperation(scope));
            }
            else {
                return result;
            }
    ...
    logicalCompareOperation(scope) {
        ...
        while (true) {
            if (this.token.value == constants_1.OPERATOR_LESSER) {
                ...
                if (this.token.value == constants_1.OPERATOR_EQUALS) {
                    this.nextToken();
                    result = new index_1.LessThanEquals(result, this.addSubOperation(scope));
                }
                else {
                    result = new index_1.LessThan(result, this.addSubOperation(scope));
                }
            }
                ...
                if (this.token.value == constants_1.OPERATOR_EQUALS) {
                    this.nextToken();
                    result = new index_1.GreaterThanEquals(result, this.addSubOperation(scope));
                }
                else {
                    result = new index_1.GreaterThan(result, this.addSubOperation(scope));
                }
            }
                ...
                if (this.token.value == constants_1.OPERATOR_EQUALS) {
                    this.nextToken();
                    result = new index_1.Equals(result, this.addSubOperation(scope));
                }
            }
                ...
                if (this.token.value == constants_1.OPERATOR_EQUALS) {
                    this.nextToken();
                    result = new index_1.NotEquals(result, this.addSubOperation(scope));
                }
                else {
                    throw new errors_1.SyntaxError('invalid syntax', this.token);
                }
    ...
    booleanNegationOperation(scope) {
        ...
        while (true) {
            if (this.token.value == constants_1.OPERATOR_NEGATE) {
                this.nextToken();
                result = new index_1.NotExpression(result);
            }
            else {
                return result;
            }
    ...
    booleanAndOperation(scope) {
        ...
        while (true) {
            if (this.token.value == constants_1.OPERATOR_AND) {
                this.nextToken();
                result = new index_1.AndExpression(result, this.booleanNegationOperation(scope));
            }
            else {
                return result;
            }
    ...
    booleanOrOperation(scope) {
        ...
        while (true) {
            if (this.token.value == constants_1.OPERATOR_AND) {
                this.nextToken();
                result = new index_1.OrExpression(result, this.booleanAndOperation(scope));
            }
            else {
                return result;
            }
    ...
    expresionSequence(scope) {
        ...
        while (true) {
            if (this.token.value == constants_1.SYMBOL_COMMA_DELIMIER) {
                ...
                return new index_1.SequenceExpression(...sequence);
            }
        }
    }
    check(constant, nextToken = true) {
        if (this.token.value !== constant)
            throw new errors_1.SyntaxError(`invalid syntax`, this.token.value);
        if (nextToken) {
            this.nextToken();
        }
